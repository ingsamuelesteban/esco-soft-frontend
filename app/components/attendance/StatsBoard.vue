<template>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center">
            ...
            <div v-if="!attendanceTaken" class="px-3 py-1 bg-gray-200 text-gray-600 dark:text-gray-400 rounded-full text-xs font-medium">
                Aún sin pasar lista
            </div>
            <div v-else class="flex items-center gap-3">
                <button @click="handlePrint" :disabled="isPrinting"
                    class="p-1 rounded-full text-gray-400 hover:text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:bg-gray-800 transition-colors"
        ...
        <div v-if="attendanceTaken" class="p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border-l-4 border-green-500">
                    <h4 class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Presentes</h4>
                    <div class="mt-2 flex item-baseline">
                        <span class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ stats?.presente?.total || 0 }}</span>
                        <span class="ml-2 text-xs text-gray-500 dark:text-gray-400" v-if="(stats?.total_registrados || 0) > 0">
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border-l-4 border-red-500">
                    <h4 class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Ausentes</h4>
                    <div class="mt-2 flex item-baseline">
                        <span class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ stats?.ausente?.total || 0 }}</span>
                        <span class="ml-2 text-xs text-gray-500 dark:text-gray-400" v-if="(stats?.total_registrados || 0) > 0">
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border-l-4 border-yellow-500">
                    <h4 class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Excusas / Tardanzas</h4>
                    <div class="mt-2 flex item-baseline gap-4">
                        <div>
                            <span class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ stats?.excusa?.total || 0 }}</span>
                            <span class="text-xs text-gray-500 dark:text-gray-400 ml-1">Exc</span>
                        </div>
                        <div>
                            <span class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ stats?.tardanza?.total || 0 }}</span>
                            <span class="text-xs text-gray-500 dark:text-gray-400 ml-1">Tard</span>
                        </div>
                    </div>
                    <div class="mt-2 flex gap-4 text-xs text-gray-600 dark:text-gray-400">
                        ...
                        <div class="flex flex-col">
                            <div class="flex gap-2">
                                <span class="text-[10px] text-gray-500 dark:text-gray-400">Exc:</span>
                                <span>{{ stats?.excusa?.masculino || 0 }} M</span>
                                <span>{{ stats?.excusa?.femenino || 0 }} F</span>
                            </div>
                            <div class="flex gap-2">
                                <span class="text-[10px] text-gray-500 dark:text-gray-400">Tard:</span>
                ...
                <!-- Retirados -->
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border-l-4 border-gray-500">
                    <h4 class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Retirados</h4>
                    <div class="mt-2 flex item-baseline">
                        <span class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ stats?.retirado?.total || 0 }}</span>
                        <span class="ml-2 text-xs text-gray-500 dark:text-gray-400" v-if="(stats?.total_registrados || 0) > 0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StudentListCard title="Retirados" :students="stats?.retirado?.estudiantes || []"
                    headerColor="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200" emptyText="Sin retirados"
                    :isGlobal="title === 'Resumen General'" />
            </div>
        </div>

        <div v-else class="p-8 text-center text-gray-500 dark:text-gray-400 italic">
            Aún sin pasar lista
        </div>
    </div>
</template>
