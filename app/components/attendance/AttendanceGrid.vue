  ...
  <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
    ...
    <div class="p-4 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center justify-between">
        <div>
          <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Lista de Asistencia</h3>
          <p class="text-sm text-gray-600 dark:text-gray-400">{{ formatFecha(fecha) }} - {{ aulaName }}</p>
        </div>
    ...
    <div v-if="attendanceStore.loading && attendanceStore.records.length === 0" class="p-8 text-center">
      <div class="flex justify-center items-center space-x-3">
        ...
        <span class="text-sm text-gray-600 dark:text-gray-400 font-medium">Cargando estudiantes...</span>
      </div>
    </div>

    <!-- Lista de estudiantes -->
    <div v-else-if="attendanceStore.records.length > 0" class="divide-y divide-gray-200 dark:divide-gray-700">
      <div v-for="record in attendanceStore.records" :key="record.estudiante.id"
        class="p-4 hover:bg-gray-50 dark:bg-gray-900/50 transition-colors">
        <div class="flex items-center justify-between">
        ...
        <div
          ...
          <textarea v-model="localObservaciones[record.estudiante.id]"
            ...
            class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md focus:ring-primary-500 focus:border-primary-500 resize-none disabled:bg-gray-100 dark:bg-gray-800 disabled:text-gray-500 dark:text-gray-400"
    ...
    <div v-else-if="!attendanceStore.loading" class="p-8 text-center">
      ...
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">No se encontraron estudiantes para esta aula.</p>
    </div>
...
const getButtonClass = (estado: string, estadoActual?: string, forcedTardiness?: boolean) => {
  ...
  return `${baseClass} bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:bg-gray-900/50`
}
