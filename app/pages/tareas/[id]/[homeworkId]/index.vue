                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="submission in submissions" :key="submission.id"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    ...
                                    <!-- Calificar: solo para entregas pendientes -->
                                    <button v-if="submission.status === 'submitted' && !isReadOnly && submission.id"
                                        @click="gradeSubmission(submission)"
                                        class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300">
                                        Calificar
                                    </button>
                                    <!-- Editar calificación: para entregas ya calificadas -->
                                    <button
                                        v-if="(submission.status === 'graded' || submission.status === 'returned') && !isReadOnly && submission.id"
                                        @click="gradeSubmission(submission)"
                                        class="inline-flex items-center gap-1 text-amber-600 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 text-sm font-medium">
