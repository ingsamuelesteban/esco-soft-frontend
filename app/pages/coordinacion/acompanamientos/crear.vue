<template>
  <div class="p-6">
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-800">Nuevo Acompañamiento - Maestro Técnico</h1>
    ...
    <div v-else class="mx-auto max-w-5xl rounded-xl bg-white p-8 shadow-sm ring-1 ring-gray-100">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between">
          <div v-for="(step, index) in steps" :key="index" class="flex flex-1 flex-col items-center">
            <div
              ...
              :class="currentStep > index ? 'border-primary-600 bg-primary-600 text-white' : (currentStep === index ? 'border-primary-600 text-primary-600' : 'border-gray-300 text-gray-400')">
        ...
        <div v-if="savingDraft" class="text-center text-xs text-gray-500 animate-pulse mt-2">
      ...
      <div v-if="currentStep >= 3 && currentStep < steps.length - 1">
        ...
        <div v-for="(section, secIndex) in currentDimensionConfig?.sections" :key="secIndex"
          ...
          <div v-for="q in section.questions" :key="q.id" class="mb-6 rounded-lg bg-gray-50 p-4 border border-gray-200">
...
async function onAssignmentSelect() {
  ...
  if (assignment && assignment.aula) {
    try {
      ...
      // Calculate counts
      const males = students.filter((s: any) => s.sexo === 'M').length;
      const females = students.filter((s: any) => s.sexo === 'F').length;
...
async function loadObservation(id: string) {
  try {
    const { data } = await useApi(`/observations/${id}`);
    const obs = data; // Assuming API returns object in data.data or root
