                    ...
                    <tr v-for="invoice in invoices" :key="invoice.id" class="hover:bg-gray-50 dark:bg-gray-900/50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button v-if="invoice.delivery_status !== 'delivered'" @click="openEditModal(invoice)"
                                title="Editar"
                                class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 mr-4 inline-flex items-center justify-center">
                            ...
                            <button @click="openManageModal(invoice)"
                                title="Gestionar"
                                class="text-primary-600 hover:text-primary-900 dark:text-primary-400 dark:hover:text-primary-300 mr-4 inline-flex items-center justify-center">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const deleteInvoice = async (id) => {
    const { isConfirmed } = await Swal.fire({
        title: '¿Estás seguro?',
        text: "Esta acción eliminará la factura. Si la factura tiene pagos o entregas, no podrá ser eliminada.",
        icon: 'warning',
    ...
    if (isConfirmed) {
        try {
            await $api.delete(`/api/student-invoices/${id}`)
            await fetchInvoices(pagination.value.current_page)
            Swal.fire({
                icon: 'success',
                title: 'Eliminada',
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: e.response?.data?.message || 'No se pudo eliminar la factura'
            })
        }
    }
}
