<template>
    <div class="h-full bg-gray-50 flex flex-col p-6">
        <div class="mb-6 flex justify-between items-center flex-shrink-0">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Facturación de Uniformes</h1>
                <p class="mt-1 text-sm text-gray-500">Gestiona las órdenes de uniformes, pagos y entregas de estudiantes
                    activos.</p>
            </div>
            <div>
        ...
        <!-- Filtros -->
        <div class="bg-white p-4 rounded-lg shadow ring-1 ring-gray-200 mb-6 flex flex-wrap gap-4 items-end">
            ...
            <div class="flex-1">
                <label class="block text-sm font-medium text-gray-700 mb-1">Estado de Entrega</label>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr v-if="(invoices || []).length === 0 && !loading">
                        <td colspan="6" class="px-6 py-8 text-center text-sm text-gray-500">No hay facturas registradas.
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="mt-4 flex items-center justify-between" v-if="pagination.last_page > 1">
            <button @click="fetchInvoices(pagination.current_page - 1)" :disabled="pagination.current_page === 1"
                class="px-4 py-2 border rounded-md text-sm bg-white hover:bg-gray-50 disabled:opacity-50">Anterior</button>
            <span class="text-sm text-gray-600">Página {{ pagination.current_page }} de {{ pagination.last_page
                }}</span>
            <button @click="fetchInvoices(pagination.current_page + 1)"
                :disabled="pagination.current_page === pagination.last_page"
                class="px-4 py-2 border rounded-md text-sm bg-white hover:bg-gray-50 disabled:opacity-50">Siguiente</button>
        </div>
                            ...
                            <DialogPanel
                                ...
                                <form @submit.prevent="submitDeliveries">
                                    ...
                                    <div class="px-6 py-4 space-y-6" v-if="selectedInvoice">
                                        <!-- Pago -->
                                        <div
                                            class="bg-gray-50 p-4 rounded-lg flex flex-col md:flex-row gap-4 items-center justify-between">
                                            <div class="flex flex-col gap-1 w-full md:w-auto">
                                                <h4 class="text-sm font-medium text-gray-900">Balance de Factura</h4>
                                                <p class="text-xs text-gray-500">Monto Total: <span
                                            ...
                                            <div class="w-full md:w-auto flex flex-col items-end justify-center">
                                                ...
                                                <div v-else class="flex flex-col gap-2 w-full md:w-auto">
                                                    <div class="flex flex-row items-center justify-end gap-2">
                                                        <span class="text-sm font-medium text-gray-700">Abonar:</span>
                                        ...
                                        <!-- Entregas -->
                                        <div>
                                            <h4 class="text-sm font-medium text-gray-900 mb-4">Artículos a Entregar</h4>
                                                    ...
                                                    <tr v-for="(detail, index) in selectedInvoice.details"
                                                        ...
                                                        <td class="px-4 py-3 text-sm text-center">
                                                            <input type="number" v-model.number="deliveries[detail.id]"
                                                                ...
                                                                class="w-20 rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm text-center disabled:opacity-50">
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
                ...
                <div class="fixed inset-0 overflow-y-auto">
                    <div class="flex min-h-full items-center justify-center p-4">
                        <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                            ...
                            leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
                            <DialogPanel
                                class="w-full max-w-4xl transform overflow-hidden rounded-xl bg-white text-left align-middle shadow-xl transition-all flex flex-col max-h-[90vh]">
                                <div
                                ...
                                <div class="flex-1 overflow-y-auto p-6 bg-gray-50 flex flex-col md:flex-row gap-6">
                                    <!-- Selección de Estudiante -->
                                    <div class="md:w-1/3 space-y-4">
                                        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                                            <h4 class="text-sm font-semibold text-gray-900 mb-3 border-b pb-2">Selección
                                                de Estudiante</h4>

                                            <div v-if="!selectedStudent">
                                                <label class="block text-sm font-medium text-gray-700 mb-1">Buscar por
                                                ...
                                                <div v-if="searchResults.length > 0"
                                                    class="mt-4 border rounded-md divide-y overflow-y-auto max-h-40 bg-white">
                                                    <div v-for="student in searchResults" :key="student.id"
                                                        @click="selectStudent(student)"
                                                        class="p-3 text-sm cursor-pointer hover:bg-gray-50 flex justify-between items-center">
                                                        <div>
                                                            <div class="font-medium text-gray-900">{{ student.nombres }}
                                                                {{ student.apellidos }}</div>
                                                            <div class="text-xs text-gray-500" v-if="student.matricula">
                                        ...
                                        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Notas
                                                (Opcional)</label>
                                    ...
                                    <div class="md:w-2/3 flex flex-col gap-4">
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                <h4 class="text-sm font-semibold text-gray-900">Artículos Disponibles
                                                </h4>
                                            </div>
                                            <ul role="list" class="divide-y divide-gray-200 max-h-48 overflow-y-auto">
                                                <li v-for="article in catalogItems" :key="article.id"
                                                    ...
                                                    <div>
                                                        <div class="font-medium text-sm text-gray-900">{{ article.name
                                                            }}</div>
                                                ...
                                                <li v-if="catalogItems.length === 0"
                                                    class="p-4 text-center text-sm text-gray-500">Cargando catálogo...
                                                </li>
                                            </ul>
                                        </div>
                                        ...
                                        <div
                                            ...
                                            <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
                                                <h4 class="text-sm font-semibold text-gray-900">Factura Actual</h4>
                                            </div>
                                                        ...
                                                        <div class="flex items-center gap-3">
                                                            <div class="flex items-center border rounded-md">
                                                                <button type="button" @click="decreaseQty(index)"
                                                                    class="p-1 px-2 text-gray-500 hover:bg-gray-50">-</button>
                                                                <span
                                                                    class="px-2 text-sm text-gray-900 min-w-[2rem] text-center">{{
                                                                        item.quantity }}</span>
                                ...
                                <div
                                    class="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 bg-gray-50 flex-shrink-0">
                                    <button type="button" @click="closeCreateModal"
                                        class="inline-flex justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
                                        Cancelar
                                    </button>
